        ...
        <Container className="space-y-6">
          {industries.map((industry, index) => (
            <FadeIn key={industry.slug} delay={(index % 4) * 60}>
              <div className="grid grid-cols-1 gap-8 rounded-2xl border border-ink-100 bg-white p-8 shadow-soft md:grid-cols-[auto_1fr_1fr] md:items-start">
                ...
                <div>
                  <h3 className="font-heading text-xl font-semibold text-ink-900">{industry.name}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-ink-500">{industry.description}</p>
                  <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-ink-400">Challenges</p>
                  <ul className="mt-2 space-y-2">
                    {industry.challenges.map((challenge) => (
                      <li key={challenge} className="flex items-start gap-2 text-sm text-ink-600">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-ink-400 md:mt-8">
                    How Taxivo Helps
                  </p>
                  <ul className="mt-2 space-y-2">
                    {industry.solutions.map((solution) => (
                      <li key={solution} className="flex items-start gap-2 text-sm text-ink-600">

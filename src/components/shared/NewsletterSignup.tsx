...
export function NewsletterSignup({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-ink-100 bg-ink-50 p-6 sm:p-8",
        className
      )}
    >
      <div className="flex flex-col items-start gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="font-heading text-xl font-bold text-ink-900">Never Miss a Deadline</h3>
        ...
        {isSubscribed ? (
          <p className="text-sm font-semibold text-accent-600">You&apos;re subscribed. Thanks for joining!</p>
        ) : (
          <form
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <div className="relative">
                ...
                <input
                  ...
                  className="h-12 w-full rounded-full border border-ink-200 bg-white pl-10 pr-4 text-sm text-ink-900 placeholder:text-ink-300 transition-colors duration-200 focus:border-brand-500 focus:outline-none sm:w-64"
                  {...register("email")}
                />
              </div>
              <button
